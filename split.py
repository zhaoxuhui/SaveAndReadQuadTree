...
def getMeanGrayAndError(img_block):
    mean = np.mean(img_block)
    img_error = np.abs(img_block - mean)
    img_error = np.mean(img_error)
    img_error = np.sqrt(img_error)
    return int(mean), img_error
...
def main():
    args = sys.argv[1:]
    if len(args) != 1:
        print 'Usage: python split.py input_image'
        return
    model = Model(args[0])
    previous = None
...
if __name__ == '__main__':
    ...
    IMG_WIDTH = 1600
    IMG_HEIGHT = 1066
    quadTree.compressAndSaveData(model, "nodes.tree")
    nodes = quadTree.decompressAndLoadData("nodes.tree")
    restore = quadTree.restoreImageFromNodes(nodes, IMG_WIDTH, IMG_HEIGHT)
    print restore[500, 600]
